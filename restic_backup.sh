...
usage() {
    ...
    echo "  -p|--prune                  Execute forget/prune only (retention maintenance)"
    echo ""
    echo "  -r and -p may be combined; at least one is required."
    echo "  ALLOW_ROOT_FS=1 env var bypasses the destination mount-point safety check."
    exit 1
}
...
# Retention maintenance only. Intended to be scheduled separately from backups (e.g. nightly
# backups, weekly prune). Does not create repos; missing repos are skipped.
prune_user() {
    local USER="$1"
    local REPO="${DESTINATION}/${USER}"
    local FORGET_OPTS="--keep-daily ${KEEP_DAILY} --keep-weekly ${KEEP_WEEKLY} --prune"
    ...
    echo "Starting prune for user: ${USER}"

    # Remove stale locks only (safe under flock; see note in backup_user).
    "${RESTIC}" unlock --repo "${REPO}" --password-file "${PASS_FILE}" > /dev/null 2>&1

    if ! "${RESTIC}" forget --repo "${REPO}" --password-file "${PASS_FILE}" ${FORGET_OPTS} > "${LOG}/${SFX}_${USER}-prune.log" 2>&1; then
